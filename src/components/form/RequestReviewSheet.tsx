...
export default function RequestReviewSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div 
        className="fixed inset-0 bg-slate-900/35 backdrop-blur-xs transition-opacity animate-in fade-in-50 duration-200"
        onClick={onClose}
      />

      {/* Floating Sheet Panel (Thụt vào trong với margin inset và bo góc rounded-2xl) */}
      <div className="fixed inset-y-2 right-2 sm:inset-y-4 sm:right-4 max-w-full flex z-50">
        <aside 
          className="w-[95vw] sm:w-[540px] md:w-[600px] bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-250 ease-out"
          role="dialog"
          aria-modal="true"
        >
          {/* 1. Header */}
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 divide-y divide-slate-100">
            ...
            <div className="space-y-3.5">
              ...
              <div className="space-y-3.5 text-xs sm:text-sm">
                ...
                {/* 3. Mô tả nhu cầu UX (Xuống dòng để dễ đọc bài toán dài) */}
                <div className="pt-1">
                ...
                {/* 5. Vấn đề người dùng (Xuống dòng) */}
                {form.user_problem && (
                  <div className="pt-1">
                    <span className="text-slate-400 font-medium block text-xs sm:text-sm mb-1">
                      Vấn đề người dùng
                ...
                {form.target_user && (
                  <div className="flex items-start">
                    ...
                    <span className="font-semibold text-slate-800 flex-1 break-words [overflow-wrap:anywhere]">
                      {form.target_user}
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-start">
                  ...
                  <span className="font-semibold text-[#1057FB] flex-1 break-words [overflow-wrap:anywhere]">
                    {form.release_date || "Chưa xác định"}
                  </span>
                </div>
            ...
            {/* ATTACHMENTS / FILE & LIÊN KẾT ĐÍNH KÈM */}
