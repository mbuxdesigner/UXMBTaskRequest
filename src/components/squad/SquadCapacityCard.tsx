...
export default function SquadCapacityCard({
  ...
  return (
    <SpotlightCard
      ...
      className="flex flex-col hover:border-[#1057FB]/40 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group bg-white relative overflow-hidden"
    >
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h4 className="font-bold text-slate-900 text-base leading-snug group-hover:text-[#1057FB] transition-colors truncate">
              {squad.squad_name}
            </h4>
        ...
        {/* Task Counts Summary */}
        <div className="grid grid-cols-3 gap-2 py-2.5 px-3 rounded-xl bg-slate-50/80 border border-slate-100 items-center text-center">
          <div>
            <p className="text-lg font-bold text-slate-900 leading-none font-mono">
              <NumberTicker value={squad.active_tasks} />
            </p>
            <p className="text-[10px] uppercase font-bold text-slate-400 mt-1">Đang làm</p>
          </div>
          <div className="border-x border-slate-200/80 px-1">
            <p className="text-lg font-bold text-slate-900 leading-none font-mono">
              <NumberTicker value={squad.queued_tasks} />
            </p>
            <p className="text-[10px] uppercase font-bold text-slate-400 mt-1">Hàng đợi</p>
          </div>
          <div className="min-w-0 px-1 flex flex-col items-center justify-center">
            <div className="flex items-center gap-1 min-w-0">
              <UserAvatar name={squad.ux_owner} size="xs" showBorder={false} />
              <span className="text-xs font-bold text-slate-800 truncate max-w-[50px]">
                {squad.ux_owner.split(" ").slice(-1)[0]}
              </span>
            </div>
        ...
        {interactive && onViewDetails && (
          <Button
            ...
            className="mt-auto w-full justify-between font-semibold text-xs rounded-xl group-hover:border-[#1057FB]/30 cursor-pointer shadow-2xs hover:bg-slate-50"
          >
            <span>Chi tiết năng lực Squad</span>
            <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#1057FB] group-hover:translate-x-0.5 transition-all" />
          </Button>
        )}
      </div>
    </SpotlightCard>
