...
export default function AppHeader({
  ...
  return (
    <>
      <header className="sticky top-0 z-40 h-14 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-6 lg:px-8 flex items-center justify-between select-none">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
          ...
          <div className="relative" ref={appsRef}>
            ...
            {/* Apps Grid Popover */}
            {appsOpen && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-2xl border border-slate-200/90 shadow-2xl p-3 z-50 animate-in fade-in-50 zoom-in-95 origin-top-right">
          ...
          <div className="relative" ref={userMenuRef}>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-[260px] bg-white rounded-2xl border border-slate-200/90 shadow-2xl p-1 z-50 animate-in fade-in-50 zoom-in-95 origin-top-right select-none">
      ...
      {searchOpen && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-start justify-center pt-20 px-4 animate-in fade-in-50"
          onClick={() => setSearchOpen(false)}
        >
          <div 
            className="w-full max-w-lg bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in-95"
      ...
      {isChangeNameOpen && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in-50"
          onClick={() => setIsChangeNameOpen(false)}
        >
          <div 
            className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-2xl p-6 space-y-5 animate-in zoom-in-95 select-none"
            onClick={(e) => e.stopPropagation()}
          >
